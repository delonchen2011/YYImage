...
#define BUFFER_MIN_SIZE (10 * 1024 * 1024) // 10MB (min memory buffer size)
#define BUFFER_MAX_SIZE (25 * 1024 * 1024) // 25MB (max memory buffer size)
...
- (void)awakeFromNib
{
    ...
    _maxBufferSize = BUFFER_MAX_SIZE;
}
...
- (void)step:(CADisplayLink *)link {
    ...
    LOCK(
         ...
         if (bufferedImage) {
             if (_incrBufferCount < (int)_totalFrameCount) {
