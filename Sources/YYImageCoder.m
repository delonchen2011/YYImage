...
static inline uint32_t YYImageByte2UInt32(const uint8_t *byte)
{
    uint32_t dst = 0;
    void *pDst = &dst;
    const void *src = byte;
    memcpy(pDst, src, sizeof(dst));
    return dst;
}
    ...
    do {
        ...
        switch (chunk->fourcc) {
            case YY_FOUR_CC('a', 'c', 'T', 'L') : {
                if (chunk->length == 8) {
                    apng_frame_number = yy_swap_endian_uint32(YYImageByte2UInt32(chunk_data + 8));
                    apng_loop_num = yy_swap_endian_uint32(YYImageByte2UInt32(chunk_data + 12));
                } else {
